...
export class PdfService {
  ...
  private async logPdf(data: Partial<PdfLog>) {
    // The PDF has already been generated by the time this is called - a
    // failure writing the audit log entry (e.g. a bad/missing field) must
    // never take down the response and lose the actual deliverable. This
    // is exactly what happened before: req.user.sub was always undefined
    // (the JWT payload's field is userId, not sub), so generatedBy - a
    // required field - was always missing, throwing an uncaught Mongoose
    // validation error on every single PDF type after the PDF itself had
    // already rendered successfully.
    try {
      await this.pdfLogModel.create(data);
    } catch (err: any) {
      this.logger.warn(`Failed to write PDF audit log (non-fatal): ${err.message}`);
    }
  }
